...
Estado → para cosas que cambian la pantalla. localStorage → para cosas que no se borran al cerrar

Componentes son pedazos de pantalla reutilizables. Props son los datos que reciben

APIs son mensajeros. Usan await y pueden fallar (siempre manejá errores)

map sirve para recorrer listas y mostrar componentes

console.log es la forma más fácil de entender qué está pasando adentro
...
1. **Crear `CONCEPTOS_BASICOS.md`** - Copiar el primer bloque de código
2. **Crear `CONCEPTOS_AVANZADOS.md`** - Copiar el segundo bloque de código
3. **Actualizar el `README.md` principal** agregando esta sección:

```markdown
## 📖 Material de apoyo

Si es tu primera vez programando con IA, leé esto primero:

👉 **[CONCEPTOS_BASICOS.md](./CONCEPTOS_BASICOS.md)** - Lo que necesitás saber desde el día 1

Cuando ya hayas hecho 2 o 3 proyectos y quieras entender más a fondo:

👉 **[CONCEPTOS_AVANZADOS.md](./CONCEPTOS_AVANZADOS.md)** - Para cuando quieras debuggear sola/o
También podés actualizar DIFICULTAD.md para que referencie estos documentos al principio o al final.
